...
def main(train_features_path,train_labels_path,test_features_path,test_labels_path):
    ...
    results = {}
    weight_0 = len(labels_train) / sum(labels_train == 0)
    weight_1 = len(labels_train) / sum(labels_train == 1)
    weight_2 = len(labels_train) / sum(labels_train == 2)
    ...
    xgb_grid = GridSearchCV(model_xgb, xgb_params, cv=3, scoring='f1_weighted', n_jobs=-1)
...
if __name__ == "__main__":
    ...
    for id in train_ids:
        predict_file(id,Path("/Users/maika/Desktop/preprocessing-geospatial-data/data/cartagraphie_des_cultures_dataset_2024-07-01_2024-10-15_monthly_data_hivernal_2024"),rf_model,xgb_model)
