...
def main():
    ...
    with open('/Users/maika/Desktop/presto/presto/dataops/data/bassin_arachidier_133_data_new_label_mapped/train.txt', 'r') as f:
        ids = [line.strip() for line in f]

    train_data=process_all_ids(ids,data_dir)
  
    label_mapping={12:0,10:1,11:1,17:2,15:2,14:2,17:2,23:2,7:2}
    ...
    def remap_labels(labels):
        ...
        _labels = torch.full_like(labels, -1)

        
        # Classe 11 devient 0
        for label in label_mapping:
            _labels[labels == label] = label_mapping[label]
        # _labels[labels == 11] = 0
        # _labels[labels == 8] = 1
        # _labels[labels == 9] = 2
        # _labels[labels == 17] = 3
    ...
    _labels = remap_labels(train_data[4].long())

    # Filtrer les données pour ne garder que les étiquettes bonnes 
    mask = _labels != -1
    ...
    dataset = TensorDataset(
        ...
        month,                        # month,
        _labels[mask].long()    # étiquettes binaires
    )
    ...
    finetuning_model=pretrained_model.construct_finetuning_model(num_outputs=num_classes, regression=True)
    ...
    with torch.no_grad():
        for (x,mask,dw,latlon,month,labels) in tqdm(val_dl):
            ...
            _, val_predicted = torch.max(outputs, 1)  # Get the predicted class index
